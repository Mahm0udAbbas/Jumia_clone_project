...
import { firestore } from './api/firstore';
...
function Cart() {
  ...
  function increaseCart(index) {
    ...
    updateDoc(doc(firestore, "cart", "wj3GhaCeRd4pnKwMZ6Ny"), { items: [...separateProducts] });
  }
                ...
                cartProducts.map((product, index) => {
                  ...
                  return <div key={product.product.id}>
                    ...
                    <div className="mb-2 flex justify-between">
                      <div className="flex">
                        <button type="button" className="flex rounded p-2 items-center space-x-2 hover:bg-orange-500/25 ">
                          <LuTrash size={18} className="text-orange-500" />
                          <span className="text-sm font-medium text-orange-500" onClick={() => { removeItemFromCart(index) }}>REMOVE</span>
                        </button>
                      </div>
                      <div className="min-w-24 flex">
                        <Button variant="gradient" size="sm" color="orange" disabled={product.quantity == 1} onClick={() => decreaseCart(index)}>-</Button>
                        <span className='mx-6'>{product.quantity}</span>
                        <Button variant="gradient" size="sm" color="orange" onClick={() => increaseCart(index)}>+</Button>
                      </div>
                    </div>
                  </div>
                })
          ...
          >
            ...
            <div className="text-green-700 border-t">
              <div className="flex gap-4 p-2">
                <Button onClick={() => router.push("/order")} variant="gradient" className="text-xl" color="orange" fullWidth> Checkout ({total}.00)</Button>
              </div>
            </div>
          </section>
        </form>
