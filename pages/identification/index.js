...
function Login_signup() {
  ...
  function loginWithGoogle() {
    signInWithPopup(auth, provider).then((result) => {
      ...
      if (storageProducts) {
        setDoc(doc(firestore, "cart", result.user.uid), { products: [...storageProducts] });
  ...
  return (
    <>
      {spinner ? <Spinner /> : ""}
      <Card className={spinner ? "flex flex-col items-center mt-10 relative opacity-25" : "flex flex-col items-center mt-10 relative"} shadow={false}>
